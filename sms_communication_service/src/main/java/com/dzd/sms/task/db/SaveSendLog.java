...
public class SaveSendLog  extends BaseTask {
    ...
    private  static LogUtil logger = LogUtil.getLogger(SaveSendLog.class );
...
//                        System.out.println("savesendlog - cols[0]====================================================》》" + cols[0]);
                      ...
                      //发送任务信息
                        ...
                        if (smsTaskDataMap.containsKey(key)) {
                            smsTaskData = smsTaskDataMap.get(key);
                        } else {
                            smsTaskData = (SmsTaskData) redisClient.hgetObject(Define.KEY_SMS_TASK_CACHE, key);
                            smsTaskDataMap.put(key, smsTaskData);
                        }
                        
                        
                        String phoneMapKey = new StringBuilder().append(smsSendResult.getTaskId()).append(",").append(smsSendResult.getAisleId()).append(",").append(smsSendResult.getState()).append(",").append(DateUtil.formatDateTime(smsTaskData.getTaskSendTime())).toString();
        ...
        try {
            if( smsSendResultList.size()>0) {
                ...
                for(String key : updateTaskPhoneMap.keySet()){
                    ...
                    updateSql = "UPDATE sms_send_task_phone  AS p SET p.state ="+parts[2]+", p.aid="+parts[1]+", p.send_time='"+parts[3]+"' WHERE p.sms_send_task_id="+parts[0]+" AND p.phone IN ("+ StringUtil.arrayToString(updateTaskPhoneMap.get(key),",")+")";
