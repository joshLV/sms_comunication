...
public class SaveReportLog extends BaseTask {
    ...
    private static LogUtil logger = LogUtil.getLogger(SaveReportLog.class);
...
//            }
                ...
                if (taskId != null ) {
                    ...
                    if (smsTaskData != null) {
                        ...
                        if (smsTaskData.getCallbackUrl() != null) {
                            ...
                            pushReportList.add(new StringBuilder().append(taskId).append(",").append(smsReport.getReceiveDate()).append(",").append(smsReport.getStateCode()).append(",").append(smsReport.getMobile()).append(",").append(smsTaskData.getSendWay()).append(",").append(smsReportPush.getPushUrl()).toString());
        ...
        try
        {
            if (pushReportList.size() > 0) {
                ...
                SmsServerManager.I.taskMap.get("SmsReportPushTask").signalNotEmpty();
            }
        } catch (
                Exception e
                )
...

        {
            if (smsMessageList.size() > 0) {
                ...
                for (String key : updateTaskPhoneMap.keySet()) {
                    ...
                    updateSql = "UPDATE sms_send_task_phone  AS p SET p.state =" + parts[2] + ",p.receive_state ='" + parts[1] + "',p.receive_code =" + parts[2] + ",p.receive_time = '" + parts[3] + "' WHERE p.sms_send_task_id=" + parts[0] + " and state=99   AND p.phone IN (" + StringUtil.arrayToString(updateTaskPhoneMap.get(key), ",") + ")  ";
