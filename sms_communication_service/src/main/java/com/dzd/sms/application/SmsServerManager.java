...
public class SmsServerManager {
    ...
    private static LogUtil logger = LogUtil.getLogger(SmsServerManager.class);
    ...
    // 特殊符号
    public List<SpecificSymbol> specificSymbolList = new ArrayList<SpecificSymbol>();
    ...
    /**
     *
     * @Title: reloadSmsTimedTak
     * @Description: 服务器重启以后需要重新构建定时任务的定时器
     * @author:    hz-liang
     * @return: void
     * @throws
     */
    @SuppressWarnings({ "unchecked", "rawtypes", "static-access" })
	private void reloadSmsTimedTak()
	{
    	String sql = "select a.id from sms_send_task a where (a.send_type=2 || (a.send_type=1 && date_format(a.timing_time, '%Y-%m-%d %H:%i') > date_format(now(), '%Y-%m-%d %H:%i')));";
    	List<Integer> taskIds = MysqlOperator.I.query(sql, new RowMapper(){
    	...
    	for(Integer taskId : taskIds){
    		SmsTaskData smsTaskData = (SmsTaskData)redisClient.hgetObject(Define.KEY_SMS_TASK_CACHE, taskId.toString());
    		if(smsTaskData == null){
    			logger.info("未能重新构建定时任务，缓存数据缺失，taskId = " + taskId);
    			continue;
    		}
    ...
    public boolean addUserBalance(Long userId, int surplus_num, String comment,String timingTag) {
        return updateUserBalance(userId, surplus_num, comment,timingTag);
    }
     ...
     * 添加或减少帐户信息
     */
    public synchronized boolean updateUserBalance(Long userId, int surplus_num,String timingTag) {
        return updateUserBalance(userId, surplus_num, "-",timingTag);
    }

    public synchronized boolean updateUserBalance(Long userId, int surplus_num, String comment,String timingTag) {
        try {
            ...
            int type = 1;
            if (surplus_num > 0) {
                if(!StringUtil.isEmpty(timingTag) && timingTag.equals("timingTag")){
                    type = 9;// 修改定时任务短信长度改动,退款，不计入到充值记录
                }else{
                    type = 2;
                }
            }
...
//        logger.info("addSmsTaskSaveSendPhoneNumber.taskId=" + taskId + ",n=" + n);
        System.out.println( " addSmsTaskSaveSendPhoneNumber.taskId = " + taskId + ",n=" + n );
     ...
     * @return
     */
    public List<SpecificSymbol> getSpecificSymbolList() {
		return specificSymbolList;
	}
     ...
     * 更新定时任务发送时间
     */
    public void updateSmsTaskSendTime(SmsTaskData smsTaskData) {
    	String Tsql = "update sms_send_task set send_time=NOW() where id=" + smsTaskData.getTaskId();
    	MysqlOperator.I.execute(Tsql);
    	
    	String Lsql = "update sms_send_log set send_time=NOW() where sms_send_task_id=" + smsTaskData.getTaskId();
    	MysqlOperator.I.execute(Lsql);
    }
    ...
    public void sendSmsTask(String taskId) {
        ...
        // 定时任务发送时，发送时间被修改延后，则不进行任何操作
        if ((smsTaskData.isFree() || smsTaskData.isAuditState())) {
        	// 更新定时任务发送时间
            updateSmsTaskSendTime(smsTaskData);
        	//通知发送任务线程
            putSmsTask(smsTaskData);
        }
        // 定时任务需要发送时状态是停止状态，则进行退款等操作
//        if (2 == smsTaskData.getTaskType()) { //没有进行审核的需要做审核失败处理，并且返还短信条数
//            //给用户返款
//            SmsServerManager.I.addUserBalance(smsTaskData.getUserId(), smsTaskData.getBilling_num(), "任务ID：" + smsTaskData.getTaskId(),null);
//            //修改数据库任务状态为终止发送
//            String Tsql = "update sms_send_task set state=9 where id=" + taskId;
//            MysqlOperator.I.execute(Tsql);
//            //存放进缓存
//            //redisClient.hsetObject( Define.KEY_SMS_TASK_CACHE , taskId, smsTaskData);
//            //放入发送队列
...
//        }
    ...
    public void delSmsTaskCache(SmsTaskData smsTaskData) {
        delSmsTaskCache(smsTaskData.getTaskId().toString() );
    }
    public void delSmsTaskCache(String taskId) {
        //删除任务缓存
        redisClient.hdelObject(Define.KEY_SMS_TASK_CACHE ,  taskId);
        //删除发送记录缓存
    ...
    public BaseAisle newBaseAisle(String className, SmsAisle smsAisle, String messageQueueKey) {
        ...
        baseAisle.baseAisleAdapter.setParams(jsonObject);
...
//        System.out.println("===========================================================>>>saveReport smid:" + c.substring(1).toString());
    ...
    public List<SmsSendLog> getSmsSendLogs(String[] aids, String phone) {
        ...
        try {
            String sql = " SELECT *  FROM sms_send_log  WHERE aid in (" + StringUtil.arrayToString(aids) + ") and receive_phone='" + phone + "'  and create_time>'" + DateUtil.formatDateTime(new Date(new Date().getTime() - 72 * 3600 * 1000)) + "' order by id desc limit 1 ";
    ...
    public void synchronizeAisle(Long id) {
        try {
            String sql = " SELECT * FROM sms_aisle   ";
            if (id > 0) {
                sql += " where id=" + id;
            }
                ...
                if( aisle.getState() == 1 ) {
                    ...
                    if (this.containsBean(aisle.getClassName())) {
                        ...
                        if (this.hasBaseAisleAdapter(aisle.getClassName())) {
                            ...
                            this.addAisle(baseAisle);
                            logger.info("add baseAisle to SmsWarehouse aisle.getClassName=" + aisle.getClassName() + " getParamValue=" + aisle.getParamValue());
                        }
                    }
                    redisClient.hsetObject(Define.KEY_CACHE_AISLE, aisle.getId().toString(), aisle);
                }else{
                    redisClient.hdelObject(Define.KEY_CACHE_AISLE, aisle.getId().toString() );
                    logger.info("del aisle id=" + aisle.getId() + " price=" + aisle.getPrice() + " option_value=" + aisle.getParamValue());
                }
            }
     ...
     * 缓存特殊符号信息
     */
    public void synchronizeSpecificSymbol() {
        try {
            String sql = "SELECT id,name  FROM sms_specific_symbol";
            //sms_specific_symbol
            //SMS_SPECIFIC_SYMBOL
            List<SpecificSymbol> specificSymbols = MysqlOperator.I.query(sql, new RowMapper() {
                public Object mapRow(ResultSet rs, int rowNum) throws SQLException {

                	SpecificSymbol s = new SpecificSymbol();
            ...
            specificSymbolList = specificSymbols;
            logger.info("synchronizeSpecificSymbol sucess ! count=" + specificSymbolList.size());
    ...
    public void sysTaskId() {
        ...
        try {
            ...
            if (redisClient.exists(Define.KEY_SMS_TASK_ID)) {
                oldId = Long.valueOf(redisClient.get(Define.KEY_SMS_TASK_ID));
            }
            //重要， 这里对比缓存中的自增ID，防止延迟插入数据库，造成任务ID重复，覆盖
            if (oldId > id && oldId - id < 1000) {
                id = oldId;
            }
            redisClient.set(Define.KEY_SMS_TASK_ID, String.valueOf(id + 100));// 正式只需要 id+2
