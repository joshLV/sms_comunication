...
function loadAisleGroup() {
    $.post(path + "/management/queryAisleGroup.do", function (data) {
...
function zy() {
    ...
    if (zyIds.length > 0) {
        if (zyIds.length > 1) {
            if (!checkLevel(zyIds)) {
                ...
                return;
            } else if (zyIds.indexOf(level_1) > -1) {
                alert("1级用户一次只能转移一条");
                return;
            } else if (zyIds.indexOf(level_2) > -1) {
                alert("2级用户一次只能转移一条");
                return;
            } else {
                $("#userType").text("");
    ...
    if (zyIds[0] == level_3) {
        //三级用户（归属---一二级用户）
        queryGSlevel(level_23);
    } else if (zyIds[0] == level_2) {
        //二级用户（归属---一二级用户）
        queryGSlevel(level_23);
    } else {
        //一级用户
        if (roleId == 3 || roleId == 58) {
            //要转移的用户是“超级管理员”和“客服”
            queryGSlevel(roleId); //如果选择的是“超级管理员”和“客服” 归属查询 "超级管理员"
        } else {
...
var checkLevel = function (zyIds) {
    ...
    for (var i in zyIds) {
        if (zyIds[i] == level_1) {
            a += 1;
        } else if (zyIds[i] == level_2) {
            b += 1;
        } else if (zyIds[i] == level_3) {
            c += 1;
        }
    }
    if (a >= 1 && (b >= 1 || c >= 1)) {
