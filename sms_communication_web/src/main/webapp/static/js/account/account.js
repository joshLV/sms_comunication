...
$(function () {
    ...
    $("#cz_allot_btn").bind("click", function () {
        ...
        var cztype = $("#type").val();
        var mn = $("#money").val();
        var checkValue=parseInt(symoney) - parseInt(-cz);

        if (cztype == "") {
            alert("请选择操作类型！");
            return;
        }
        if (cz == "" || cz == 0 || cz > 0 &&(cztype==5 ||cztype==8 || cztype ==7)) {
            alert("充值条数不能为空,并且要小于0！");
            return;
        }
        if (cz == "" || cz == 0 || cz < 0 &&(cztype !=5 && cztype!=8 && cztype !=7)) {
            alert("充值条数不能为空,并且大于0！");
            return;
        }

        if ((cztype == 5 || cztype == 8 || cztype == 7) && checkValue < 0) {
            alert("当前短信数量不足,请核对操作类型和数量！");
            return;
        }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row) {
                    return loadBtn(row.sysMenuBtns, row.id, row.email, row.surplusNum, row.state, row.sysUserId, row.checkState);
                }
            }],
