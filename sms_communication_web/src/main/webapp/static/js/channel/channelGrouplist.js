            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    if (!row.state || row.state == 1) {
                        ...
                        return "<span style='color:#fd0000'>停用</span>";
                    }
                }
            },
            {
...
$('#add_data_div').on('hidden.bs.modal', function (e) {
    $("#upload_form")[0].reset();
})
...
function init() {
    ...
    $("#i_oneIntervalEnd").blur(function () {
        var oneStartValue = $("#i_oneIntervalStart").val();
        var thisValue = $(this).val();
        if (parseInt(thisValue) <= parseInt(oneStartValue)) {
            alert("第一区间截止条数需大于" + oneStartValue + "条");
        }
    });

    // 第二区间校验
    $("#i_twoIntervalStart").blur(function () {
        var oneEndValue = $("#i_oneIntervalEnd").val();
        var thisValue = $(this).val();
        if (parseInt(thisValue) <= parseInt(oneEndValue)) {
            alert("第二区间起始条数需大于" + oneEndValue + "条");
        }
    });
    $("#i_twoIntervalEnd").blur(function () {
        var twoStartValue = $("#i_twoIntervalStart").val();
        var thisValue = $("#i_twoIntervalEnd").val();
        if (parseInt(thisValue) <= parseInt(twoStartValue)) {
            alert("第二区间截止条数需大于" + twoStartValue + "条");
        }
    });

    // 第三区间校验
    $("#i_threeIntervalStart").blur(function () {
        var twoEndValue = $("#i_twoIntervalEnd").val();
        var thisValue = $("#i_threeIntervalStart").val();
        if (parseInt(thisValue) <= parseInt(twoEndValue)) {
            alert("第三区间起始条数需大于" + twoEndValue + "条");
        }
    });
    $("#i_threeIntervalEnd").blur(function () {
        var threeStartValue = $("#i_threeIntervalStart").val();
        var thisValue = $("#i_threeIntervalEnd").val();
        if (threeStartValue != null && threeStartValue != "") {
            if (parseInt(thisValue) <= parseInt(threeStartValue)) {
                alert("第三区间截止条数需大于" + threeStartValue + "条");
            }
        }
...
function checkFormInput() {
    ...
    var smsLength = $("#smsLength").val();
    if (smsLength == null || smsLength == "") {
        alert("请输入长短信计费条数!");
        return false;
    }

    var i_oneIntervalPrice = $("#i_oneIntervalPrice").val();
    if (i_oneIntervalPrice == null || i_oneIntervalPrice == "") {
        alert("请输入第一区间价格!");
        return false;
    }
    var i_oneIntervalStart = $("#i_oneIntervalStart").val();
    if(i_oneIntervalStart == null || i_oneIntervalStart == ""){
        alert("请输入第一区间起始条数!");
        return false;
    }
    var i_oneIntervalEnd = $("#i_oneIntervalEnd").val();
    if(i_oneIntervalStart == null || i_oneIntervalStart == ""){
        alert("请输入第一区间截止条数!");
        return false;
    }

    var i_twoIntervalPrice = $("#i_twoIntervalPrice").val();
    if(i_twoIntervalPrice == null || i_twoIntervalPrice == ""){
        alert("请输入第二区间价格!");
        return false;
    }
    var i_twoIntervalStart = $("#i_twoIntervalStart").val();
    if(i_twoIntervalStart == null || i_twoIntervalStart == ""){
        alert("请输入第二区间起始条数!");
        return false;
    }
    var i_twoIntervalEnd = $("#i_twoIntervalEnd").val();
    if(i_twoIntervalEnd == null || i_twoIntervalEnd == ""){
        alert("请输入第二区间截止条数!");
        return false;
    }


    var i_threeIntervalPrice = $("#i_threeIntervalPrice").val();
    if(i_threeIntervalPrice == null || i_threeIntervalPrice == ""){
        alert("请输入第三区间价格!");
        return false;
    }
    var i_threeIntervalStart = $("#i_threeIntervalStart").val();
    if(i_threeIntervalStart == null || i_threeIntervalStart == ""){
        alert("请输入第三区间起始条数!");
        return false;
    }

    var oneStartValue = $("#i_oneIntervalStart").val();
    var thisValue = $("#i_oneIntervalEnd").val();
    if (parseInt(thisValue) <= parseInt(oneStartValue)) {
        alert("第一区间截止条数需大于" + oneStartValue + "条");
        return false;
    }
    ...
    var oneEndValue = $("#i_oneIntervalEnd").val();
    var thisValue = $("#i_twoIntervalStart").val();
    if (parseInt(thisValue) <= parseInt(oneEndValue)) {
        alert("第二区间起始条数需大于" + oneEndValue + "条");
        return false;
    }
    ...
    var leng=$("input[name='operatorId']:checked");
    if(leng.length<1){
        alert("请选择通道组!");
        return false;
    }
