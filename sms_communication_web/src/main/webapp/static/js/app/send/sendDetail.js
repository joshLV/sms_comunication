        ...
        columns: [
            {
                ...
                formatter: function (value, row, index) {
                    if (index == 0) {
                        ...
                        var date1 = row.createTimeStr;
                        var date2 = row.nowTimeStr;
                        if (date1 == date2) {// 当天显示发送中和已发送
                            $("#today_state").show();
                            $("#sendType option[value='-1']").show();
                            $("#sendType option[value='-2']").show();
                            $(".fszsum").html(row.sendingNum);// 发送中
                            $(".yfssum").html(row.sendFinishNum);// 已发送
                        } else {// 历史不显示发送中和已发送
                            $("#today_state").hide();
                            $("#sendType option[value='-1']").hide();
                            $("#sendType option[value='-2']").hide();
                        }
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    var time = ' - ';
                    if (row.createTime != null && row.sendType == 0) {
                        time = row.createTime.split(".")[0];
                    } else if (row.timing != null && row.sendType == 1 || row.sendType == 2) {
                        time = row.timing.split(".")[0];
                    }
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    if (row.feedbackTime != null) {
                        feedbackTime = row.feedbackTime.split(".")[0];
                    }
                    return feedbackTime;
                }
...
function export_phone() {
    ...
    var uri = path + "/smsUser/export/csv.do?id=" + id + "&phone=" + phone + "&type=" + type;
...
function back() {
    var data = {
        ...
        startInput: $("#startInput").val(),//开始时间
        endInput: $("#endInput").val(),//结束时间
        sendType: $("#sendType1").val(),// 1：定时任务
        groupId: $("#groupId").val(),// 通道类型id
        id: $("#id_input").val(),
        menuId: $("#menuId").val()
    }
    var uri = path + '/smsUser/sendListview.do?data=' + encodeURIComponent(JSON.stringify(data));
    window.location.href = uri;
}