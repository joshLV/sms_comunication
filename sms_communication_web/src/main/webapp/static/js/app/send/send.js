...
$(function () {
    ...
    $("#defalut_send_time").click(function () {
        ...
        $("#defalut_send_time").prop("checked", true)
        $("#check_send_time").prop("checked", false);
    });
...
$("#upBtn").bind("click",function(){
    var uuid = $("#uuid").val();
    var url=  path + '/smsUser/fileUpload.do?uuid=' + uuid;
...
function sendBeforeVerify() {
 ...
 /*   var aisle_unregTypeId = $("#aisle_unregTypeId").val();
    if(aisle_unregTypeId == 1){
        if( !lastStyle(content)){//回复退订格式判断
            alert("短信内容结尾须按要求加退订格式。")
            return;
        };
    }*/
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (code == 0) {// 检验通过
                ...
                if (isContains(msg, "短信内容中含有敏感词")||isContains(msg, "短信内容中含有特殊符号")) {
...
function getVerCode() {
    ...
    $.post(path + "/smsVertifyCode/getVertifyCodeBySend.do", data, function (data) {
...
function sendCheckVerify(){
    // 验证码弹框显示,则校验验证码
    if(!$("#verifyModal").is(":hidden")){
        checkVerifyCode();//直接调用发送短信函数
    }else{// 50条以下不需要校验验证码
        sendSms();
    }
}
...
function sendSms() {
    ...
    $("#sendsmsyz-btn").addClass("f-btn-dis");
    $.ajax({
        ...
        complete: function () {
            $("#sendsmsyz-btn").removeClass("f-btn-dis");
            $("#sendsmsyz-btn").removeAttr("disabled");
        }
    });
...
function rebind() {
    ...
    $("#smssendBtn").bind("click", function () {
        $(this).attr("disabled", "disabled");
        Button_Click(checkVerifyCode, 200);//确认发送按钮
    });
    ...
    $("#sendsmsyz-btn").bind("click", function () {
        $(this).attr("disabled", "disabled");
        Button_Click(sendCheckVerify, 200);//发送验证按钮
    });
