...
$(function () {
    ...
    if($("#id_input").val()!=""){
        $("#account_input").val($("#account").val());
        $("#start_input").val($("#startInput").val());
        $("#end_input").val($("#endInput").val());
        $("#sendType").val($("#sendType1").val());
        $("#sel_groupId").val($("#groupId").val());
    }else{
    ...
    $('#fs-table').bootstrapTable({
        ...
        columns: [
            ...
            },
            {
                ...
                width: 140,
                formatter: function (value, row, index) {
                    return new Date(row.actualSendTime).Format("yyyy-MM-dd hh:mm:ss");
                }/*,
                formatter: function (value, row, index) {
                    var time = ' - ';
                    if (row.sendType == 1 || row.sendType == 2) {
                        time = new Date(row.timingTime).Format("yyyy-MM-dd hh:mm:ss");
                    }else{
                        time = new Date(row.createTime).Format("yyyy-MM-dd hh:mm:ss");
                    }
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    if (row.sendType == 0) {
                        return "实时";
                    } else if (row.sendType == 1 || row.sendType == 2) {
                        return "定时";
                    }
                }
            },
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var f = '<a href="javaScript:void(0);" data-clipboard-action="copy" data-clipboard-target="#c-index' + index + '" id="copy-btn' + index + '" onclick=\'copyContent(this,\"' + index + '\");\'>复制</a><textarea style="filter:alpha(opacity=0);position:absolute;z-index:-100;opacity:0;width:0px;left:1px;"  id="c-index' + index + '">' + row.content + '</textarea>';
                    return q + f;
                }
            }
        ],
...
function smsSendDetails(id) {
    ...
    var uri = path + '/smsUser/sendDetailsView.do?data='+ encodeURIComponent(JSON.stringify(data));
