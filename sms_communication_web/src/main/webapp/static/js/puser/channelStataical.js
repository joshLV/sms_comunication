...
function orderExport(path) {
    ...
    ajaxCall({
        url: '/puser/aliseStatisticalList.do?menuId=' + $("#menu_id").val(),
        type: "post",
...
function load_data() {
    $('#tb_data').bootstrapTable({
        url: 'aliseStatisticalList.do?menuId=' + $("#menu_id").val(),
        dataType: "json",
        cache: false,
        striped: true,
        pageSize: 50,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    if (index == 0) {
                        if (row.sumBillingNum == 0) {
                            return 0 + "%";
                        }
                        return Math.round((row.sumSucceedNumUs + row.sumSucceedNumMs + row.sumSucceedNumTs + row.sumUnknownSucceedNum) / row.sumBillingNum * 10000) / 100.00 + "%";
                    } else {
                        if (row.billingNum == 0) {
                            return 0 + "%";
                        }
                        return Math.round((row.succeedNumUs + row.succeedNumMs + row.succeedNumTs + row.unknownSucceedNum) / row.billingNum * 10000) / 100.00 + "%";
                    }
                }
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    if (index == 0) {
                        ...
                        return Math.round((row.sumUnknownNumUs + row.sumUnknownNumMs + row.sumUnknownNumTs + row.sumUnknowNnknownNum) / row.sumBillingNum * 10000) / 100.00 + "%";
                    } else {
                        if (row.billingNum == 0) {
                            return 0 + "%";
                        }
                        return Math.round((row.unknownNumUs + row.unknownNumMs + row.unknownNumTs + row.unknowNnknownNum) / row.billingNum * 10000) / 100.00 + "%";
                    }
                }
