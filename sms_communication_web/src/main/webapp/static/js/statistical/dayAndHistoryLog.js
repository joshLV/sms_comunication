...
function orderExport() {
    ...
    $.post(
        "queryOrderExport.do?logTime=" + $("#logTime").val(),
        data,
        function (data) {
            if (data["retCode"] == "000000") {
                window.location.href = "orderExport.do?logTime=" + $("#logTime").val();
            } else {
                alert("没有可以导出数据");
            }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    return row.actualSendTime.split(".")[0];
                }
/*            formatter: function (value, row, index) {
            	if (row.sendType == 0) {
            		if (!!row.createTime) {
...
*/            },
                ...
                formatter: function (value, row, index) {
                    if ("today" == $("#logTime").val()) {
                        if (row.state == null) {
                            ...
                            return "发送中";
                        } else if (row.state == 0 || row.state == 99) {
                            return "已发送";
                        ...
                        if (row.state == null) {
                            return null;
                        } else if (row.state == 100 || row.state == 99 || row.state == 0) {
                            return "发送成功";
                        } else {
                            return "发送失败";
                        }
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    var feedbackTime = '-';
                    if (row.feedbackTime != null && row.feedbackTime != "") {
        ...
        responseHandler: function (a) {
            ...
            if (btn_arr1 != null && btn_arr1.length > 0) {
                ...
                for (var k = 0; k < btn_arr1.length; k++) {
                    ...
                    if (btn_obj["actionUrls"] == user_path + '/statusReport.do') {
                        $('#tb_data').bootstrapTable('showColumn', 'fkState');
                    }
                    if (btn_obj["actionUrls"] == user_path + '/statusTime.do') {
