...
public class SmsServiceV4 {
    ...
    static LogUtil logger = LogUtil.getLogger(SmsServiceV4.class);
     ...
     * @return
     */
    public JSONObject send(JSONObject result, SmsRequestParameter requestParameter, CustomParameter parameter, ServiceBeanUtil serviceBean) {
        // 1. 初始化返回结果
        ...
        // 4. 各种校验
        UserSendMassSmsService sendMassSmsService = new UserSendMassSmsService();
        JSONObject verifyJsonObject = sendMassSmsService.sendBeforVerify(requestParameter, serviceBean);
        if (!jsonReturnBoolean(verifyJsonObject)) {
        ...
        SmsSendBusiness.filterPhoneNumber(requestParameter, result, smsMessageList, validMobileList, parameter);
        ...
        parameter.setTiming(parameter.getTiming());
