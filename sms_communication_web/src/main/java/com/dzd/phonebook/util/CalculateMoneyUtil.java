...
public class CalculateMoneyUtil {
    ...
    public static Float calculateSmsNumMoney(Integer smsNum, SmsAisleGroup smsAisleGroup) {
        ...
        try {
            ...
            } else if (threeIntervalEnd != null && threeIntervalEnd != 0) {// 第三区间不为空,则按正常逻辑
                if (smsNum >= threeIntervalStart && smsNum <= threeIntervalEnd) {// 第三区间价格段
                    money = smsNum * threeIntervalPrice;
                } else if (smsNum > threeIntervalEnd) {// 大于第三区间，按第三区间最大价格来算
                    money = smsNum * threeIntervalPrice;
                }
            }else if (threeIntervalEnd == null || threeIntervalEnd == 0){// 第三区间最大价格为空 则按为第三区间最小的价格来算
                if (smsNum >= threeIntervalStart){
                    money = smsNum * threeIntervalPrice;
                }
            }
