...
//		bea.put("icon", "BEA_OUT.gif");
     ...
     * @return
     */
    public static String getOrderNumber(String account) {
        String orderId =  account + getCurrentTimeMillis();//2017 07 21 19 33 12
        if(orderId.length()>=30){// 快钱规定订单号长度不能超过30
    ...
    private static String getCurrentTimeMillis(){
        String nowTime = "";
        try {
            nowTime = DateUtil.getNowLongTime() + RandomUtil.getRandomTenThousand();
        }catch (Exception e){
            e.printStackTrace();
        }
        return nowTime;
