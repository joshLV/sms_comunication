...
public class SendSmsRequestParameterUtil {
	 ...
	 * @return
	 */
	public static SmsRequestParameter sendSmsRequestParameter(Map<String, Object> data, HttpServletRequest request,
			UserMessageService userMessageService, ChannelService channelService,
			SmsFileConfigService smsFileConfigService) {
		...
		try {
			...
			// 2. 获取通道信息
			Integer aisleGroupId = 0;
			if (smsUsers != null) {
				aisleGroupId = smsUsers.getAisleGroupId();
			}

			SmsAisleGroup smsAisleGroup = channelService.querySmsAisleGroupById(aisleGroupId);
			String dredgeAM = smsAisleGroup.getDredgeAM();// 开通时间段 开始时间
			String dredgePM = smsAisleGroup.getDredgePM();// 开通时间段 截止时间
			Integer unregTypeId = smsAisleGroup.getUnregTypeId(); // 退订格式
																	// 0-关，1-开
			...
			Map<String, Object> validPhoneMap = FileUploadUtil.getPhoneMap(phoneList,
					data.get("operators") == null ? "" : data.get("operators").toString());
			...
			Long groupTypId = 0l;
			if(smsUsers!=null){
				groupTypId = Long.valueOf(smsUsers.getAisleGroupId());
			}
			...
			parmeter.setVerifyType(smsUsers.getVerifyType());// 是否为首次登陆，超过50条需要验证码
																// 0:需要,1：不需要
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @param smsUserSign
	 */
	public static void initSmsRequestParameter(SmsRequestParameter requestParameter, ChannelService channelService,
			com.dzd.sms.service.data.SmsUser smsUsers, String smsUserSign) {
		...
		Date timing = null;
		int sendType = 0;// 立即发送
		if (requestParameter.getTiming() != null) {
			sendType = 1;// 定时发送
			String timeStr = requestParameter.getTiming().toString();
			SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			try {
				timing = dateFormat.parse(timeStr);
				requestParameter.setTiming(timing);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}

		requestParameter.setGroupTypeId(groupTypId);
		requestParameter.setSmsUserSign(smsUserSign);
		requestParameter.setDredgeAM(dredgeAM);// 通道开启时间
		requestParameter.setDredgePM(dredgePM);// 通道结束时间
		requestParameter.setUnregTypeId(unregTypeId);// 退订格式 0-关，1-开
		requestParameter.setSendType(sendType);// 发送类型
		requestParameter.setSmsUserState(smsUsers.getState());// 用户状态 0：启用；1：禁用
		requestParameter.setAisleGroupState(state);// 通道组状态 1：启用 2：停用
		requestParameter.setAisleLongNum(smsAisleGroup.getSmsLength());// 通道组长短信计费
		requestParameter.setShieldingFieldId(smsAisleGroup.getShieldingFieldId());// 通道组敏感词id
		requestParameter.setVerifyType(smsUsers.getVerifyType());// 是否为首次登陆，超过50条需要验证码
																	// 0:需要,1：不需要
	}
