...
public class VertifyCodeService<T> extends BaseService<T> {
     ...
     * @return
     */
    public JSONObject findBackPwd(VertifyCode v,Object sessionImgcode) {
        JSONObject json = new JSONObject();
        try {
            boolean flag = true;
            int code = 0;
            ...
            if (flag) {
                VertifyCode code2 = vertifyCodeDao.queryCodeByPhoneAndCode(v.getPhone(), verifyCode);
                if(sessionImgcode != null && !"".equals(sessionImgcode.toString()) &&
                        !imgCode.toString().trim().toUpperCase().equals(sessionImgcode.toString().trim().toUpperCase()) ){
                    code = 1;
                    flag = false;
                    msg = "图文验证码错误!";
                }else if (code2 == null) {
                    ...
                    if (bean != null) {
                        SmsUser smsUser=smsUserService.queryByEmailAndPhone(v.getEmail(), v.getPhone());
                        if (smsUser == null) {
                            code = 1;
                            flag = false;
                            msg = "用户名与号码不匹配!";
