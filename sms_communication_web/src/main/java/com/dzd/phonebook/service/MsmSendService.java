...
public class MsmSendService<T> extends BaseService<T> {
    ...
    public SmsSendLog querySumSendNum(SmsSendLog smsSendLog) {
        return getDao().querySumSendNum(smsSendLog);
    }
    ...
    public List<String> queryAisleNames(Integer smsId) {
        return getDao().queryAisleNames(smsId);
    }
    
    public List<String> querySmsAisleNames() {
    	return getDao().querySmsAisleNames();
    }

    public List<String> querynickNames(Integer smsId) {
        return getDao().querynickNames(smsId);
    }
     ...
     * @return
     */
    public Page<SmsSendLog> querySmsTimedTask(List<SysMenuBtn> sysMenuBtns, DzdPageParam dzdPageParam) {
        ...
        if (sysMenuBtns != null) {
            for (SysMenuBtn sysMenuBtn : sysMenuBtns) {
                //  通道类型列
                if (sysMenuBtn.getActionUrls().equals(Define.MENUURLACTION.TIMING_BELONG_COLUMN_URL)) {
