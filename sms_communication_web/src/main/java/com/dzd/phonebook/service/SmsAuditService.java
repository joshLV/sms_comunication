...
public class SmsAuditService<T> extends BaseService<T> {
    private final static LogUtil log = LogUtil.getLogger(SmsAuditService.class);
    ...
    public Integer rollbackNum(SmsSendTask smsSend, Integer sysUid) {
        Integer num = 1;
        try {
            Integer useBalance = SmsServerManager.I.getUserSurplusNum(Long.parseLong(smsSend.getSmsUserId().toString()));// redis缓存获取账号余额信息
