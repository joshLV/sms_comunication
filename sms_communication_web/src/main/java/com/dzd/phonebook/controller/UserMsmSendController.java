...
public class UserMsmSendController extends WebBaseController {
	public static final LogUtil log = LogUtil.getLogger(UserMsmSendController.class);
	...
	@ResponseBody
	public DzdResponse puserList(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			Page<SmsSendTask> dataList = userMsmSendService.queryMsmSendList(null, dzdPageParam);
	...
	public void exportCsv(HttpServletRequest request, HttpServletResponse response) {
		...
		map.put("phone", null);
		map.put("type", null);
		projects = userMsmSendService.queryMsmSendPhoneByid(map);
	...
	// btnFile对应页面的name属性
	@ResponseBody
	public DzdResponse fileUpLoad(@RequestParam MultipartFile[] btnFile, HttpServletRequest request,
		...
		try {
			...
			/*
			 * 保存文件 InputStream is =
			 * btnFile[0].getInputStream();//多文件也适用,我这里就一个文件 //String fileName =
			 * request.getParameter("fileName"); String guid =
			 * request.getParameter("guid"); byte[] b = new
			 * byte[(int)btnFile[0].getSize()]; int read = 0; int i = 0;
			 * while((read=is.read())!=-1){ b[i] = (byte) read; i++; }
			 * is.close(); OutputStream os = new FileOutputStream(new
			 * File("D://"+guid+"."+btnFile[0].getOriginalFilename()));//文件原名,如a
			 * .txt os.write(b); os.flush(); os.close();
			 */
