...
public class UserMessageAppController<T> extends WebBaseController {
	...
	public DzdResponse puserStatisticalList(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			// 排序
			/*
			 * if (parameters.getSort() != null && parameters.getOrder() !=
			 * null) { sortMap.put("sortVal", "order by " + parameters.getSort()
			 * + " " + parameters.getOrder()); } else { sortMap.put("sortVal",
	...
	public DzdResponse puserBillList(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if (bid != null && !StringUtil.isEmpty(bid.toString())) {
				sortMap.put("bid", Integer.parseInt(bid.toString()));
			}
	...
	@ResponseBody
	public DzdResponse puserConsumeBillList(HttpServletRequest request, @RequestBody Map<String, Object> data)
			throws Exception {
		DzdResponse dzdPageResponse = new DzdResponse();
		try {
			// 1. 基本参数设置
