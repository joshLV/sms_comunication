...
public class PayController {
    public static final LogUtil logger = LogUtil.getLogger(PayController.class);
    @Autowired
    ...
    public String saveRechargeOrder(HttpServletRequest request) {
        try {
            ...
            Object account = request.getParameter("account");// 充值用户
            ...
            if (account == null || account.equals("") || account.equals(" ")) {
                SmsUser smsUser = userMessageService.querySmsUserById(sysUser.getId());
                uid = smsUser.getId();
                accountStr = smsUser.getEmail();
            } else {

                SmsUser smsUser = userMessageService.querySmsUserBySmsUserAccount(account.toString());
                uid = smsUser.getId();
            ...
            SmsAisleGroup smsAisleGroup = smsAisleGroupService.querySmsAisleGroupByIdByPay(accountStr, request);
            ...
            smsRechargeOrder.setOrderNo(RechargeVariable.getOrderNumber(accountStr));
            ...
            String amount = Math.round(smsRechargeOrder.getMoney() * 10 * 10) + "";// 将元转换为分
