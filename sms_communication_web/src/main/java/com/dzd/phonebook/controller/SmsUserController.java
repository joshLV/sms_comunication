...
public class SmsUserController {
	...
	public DzdResponse findBackPwd(HttpServletRequest request) throws Exception {
		...
		try {
			...
			// 3. 返回结果
			JSONObject json = vertifyCodeService.findBackPwd(v,randomString);
			dzdResponse.setData(json);
		} catch (Exception e) {
			e.printStackTrace();
			dzdResponse.setRetCode(ErrorCodeTemplate.CODE_FAIL);
	...
	public Object getVertifyCode(HttpServletRequest request) throws Exception
	{
		...
		try
		{
			...
			SmsUser smsUser=smsUserService.queryByEmailAndPhone(email,phone);
			if(smsUser==null){
				response.setData(ErrorCodeTemplate.MSG_EMAIL_PHONE_ERROR);
				response.setRetCode(ErrorCodeTemplate.CODE_FAIL);
				return response;
			}
			if (StringUtils.isNotBlank(imgCode))
			{
				...
				HttpSession session = request.getSession();
				Object randomString = session.getAttribute("randomString");
				if (randomString != null && !"".equals(randomString.toString()) &&
						!imgCode.toString().trim().toUpperCase().equals(randomString.toString().trim().toUpperCase()) )
				{
			...
			List<VertifyCode> todayCodeList = vertifyCodeService.getCodeCountByPhoneAndType(phone,ErrorCodeTemplate.VERTIFYCODE_TYPE);
			if ( todayCodeList != null && todayCodeList.size() >= ErrorCodeTemplate.VERTIFYCODE_TYPE )
			{
