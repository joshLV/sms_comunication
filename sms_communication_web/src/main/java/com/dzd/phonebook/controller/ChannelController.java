...
public class ChannelController extends WebBaseController {
	public static final LogUtil log = LogUtil.getLogger(ChannelController.class);
	...
	public String merge(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		try {
			...
			if (smsAisle.getId() == null) {
				...
				// 保存操作日志
				content = ErrorCodeTemplate.CHANNEL_ADD;
				...
				// 保存操作日志
				content = ErrorCodeTemplate.CHANNEL_REGIST;
			}
			// 保存操作日志
	...
	public String groupmerge(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		try {
			...
			if (smsAisleGroup.getId() == null) {
				...
				// 保存操作日志
				content = ErrorCodeTemplate.CHANNEL_GROUP_ADD;
				...
				// 保存操作日志
				content = ErrorCodeTemplate.CHANNEL_GROUP_REGIST;
			}
			// 保存操作日志
			CommonRoleServiceUtil.saveOperateLog(user, null, content, sysUserService);
			// 查询通道开通的时段
			...
			SmsAisleGroup smsAisleGroups = channelService.querySmsAisleGroupById(smsUsers.getAisleGroupId());
			SessionUtils.setSmsAisleGroup(request, smsAisleGroups);
		} catch (Exception e) {
			log.error(null, e);
			e.printStackTrace();
		}
		return "redirect:/channel/grouplistview.do?id=" + menuId + "#submit";
	}
	 ...
	 * @return
	 */
	@RequestMapping("/getChannelByUserId")
	@ResponseBody
	public DzdResponse getChannel(HttpServletRequest request) {
		DzdResponse dzdResponse = new DzdResponse();
		try {
			SysUser user = SessionUtils.getUser(request);
